...
const colorStyles: Record<ColorKey, { bg: string; text: string; bar: string }> = {
  ...
  secondary: { bg: "bg-secondary-light", text: "text-secondary", bar: "bg-secondary" },
  warning: { bg: "bg-warning-light", text: "text-warning", bar: "bg-warning" },
  error: { bg: "bg-error-light", text: "text-error", bar: "bg-error" },
};

const trendStyles: Record<NonNullable<MetricCardProps["trend"]>["type"], string> = {
...
export function MetricCard({
  ...
  const content = (
    <Card className={cn("relative overflow-hidden hover:shadow-card-hover transition-all duration-300", className)}>
      <a href={href} className="block p-6 hover:bg-slate-50/50 transition-colors">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className={cn("flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center", styles.bg, styles.text)} aria-hidden="true">
            {icon}
          </div>
        </div>
        {/* Accent bar at bottom */}
        <div className={cn("absolute bottom-0 left-0 right-0 h-1", styles.bar)} aria-hidden="true" />
      </a>
    </Card>
  );
